...
void TransformSystem::AddChildren(Entity* child, Entity* parent)
{
    Transform& transformChild = m_gameManager->GetComponant<Transform>(child);
    Transform& transformParent = m_gameManager->GetComponant<Transform>(parent);
    if (transformChild.id == transformParent.id) return;
	transformParent.m_children.push_back(&transformChild);
    //transform.m_children.push_back(&transform);
}
